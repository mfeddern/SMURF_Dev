...
# UPDATED: 04/29/2026 BY M. FEDDERN
...
ocean<-read.csv('Data/Raw_not_confidential/Oceanography_DailyMean.csv')
mooring_central<-read.csv("Data/Raw_not_confidential/mooring_locations_central.csv") # central moorings 
settle_central<-read.csv("Data/Raw_not_confidential/SFLA_settlement_central.csv")


mooring_south<-read.csv("Data/Raw_not_confidential/mooring_locations_south.csv") # south moorings 
settle_south<-read.csv("Data/Raw_not_confidential/SFLA_settlement_south.csv") # south settlement
...
odfw_moorings<-read.csv("Data/Raw_not_confidential/odfw moorings.csv")
head(odfw_moorings)
...
# making a plot for the manuscript

ggplot(ocean_full%>%filter(Site.Code=="RR"&julian<300), 
       aes(x = julian, y = temp_index))+
  geom_line()+ 
  ggtitle("Redfish Rocks")+
  #geom_line(aes(y=scale(cdd_16)), col="blue")+
 # geom_line(aes(y=scale(cdd_8)), col="red")+
  geom_line(aes(y=rolling16d), col="green")+
  geom_line(aes(y=rolling8d), col="purple")+
  geom_line(aes(y=scale(cdd_16)), col="blue")+
  geom_line(aes(y=scale(cdd_8)), col="red")+
  theme_classic()+
  facet_wrap(~year)

ggplot(ocean_full%>%filter(Site.Code=="ORF"&julian<300), 
       aes(x = julian, y = temp_index))+
  geom_line()+ 
  ggtitle("Otter Rock")+
  #geom_line(aes(y=scale(cdd_16)), col="blue")+
...
write.csv(settle_ocean,"Data/Processed/combined_settlement_oceanV2.csv",row.names = F)
