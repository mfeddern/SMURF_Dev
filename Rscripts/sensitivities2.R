...
SS_write(sensi_mod, file.path(model_directory, 'sensitivities', 'oceanographic_index2'), overwrite = TRUE)
# 
out <- SSgetoutput(dirvec = c(file.path(model_directory, base_model_name),
                              file.path(model_directory, 'sensitivities', 'oceanographic_index'),
                               file.path(model_directory, 'sensitivities', 'oceanographic_index2')))
 SSsummarize(out) |>
  SSplotComparisons(subplots = c(1,3), endyrvec = 2037, new = FALSE)
# miraculously, it is not bad.
# 
...
colnames(SpawningBiomass)<- c("SMURF", "NoIndices", "OCNMS", "RREASN", "Oceanographic", "RREAS", "Label", "Year")
SpawningData<-SpawningBiomass%>%pivot_longer(c(SMURF, NoIndices, OCNMS, RREASN, Oceanographic, RREAS))%>%
...
#all_ts
pdf(file = "Figures/SMURF_Manuscript/spawnplot.pdf", width =8, height =6)
spawnshort
dev.off()

ggsave("Figures/SMURF_Manuscript/spawnplot.png",  dpi = 300,  
...
pdf(file = "Figures/SMURF_Manuscript/yoyindices.pdf", width =9, height =6)
indices
dev.off()

ggsave("Figures/SMURF_Manuscript/yoyindices.png",  dpi = 300,  
