...
# UPDATED: 01/22/2025 BY A. WHITMAN
...
# SUBSET TO MID-WATER DEPTHS ONLY FOR INTIAL DATASET, THEN PRIORITIZING DATA FROM SURFACE AND THEN DEEP TO REDUCE NA'S

ocean_merge<-ocean[ocean$depth_cat=="mid",]
...
str(settle)
str(ocean_merge) # note that I'm using the mid-water only depths test dataset, 
#then I'll pull in the other ocean data to fill in holes
...
settle_ocean<-merge(settle, ocean_merge[,c(8:12)], by = c("ID"), all.x = TRUE)

names(settle_ocean)
head(settle_ocean)
summary(settle_ocean)

# pull in data from shallow or deep water temps to fill in the oceanographic holes as much as possible

ocean_shallow<-ocean[ocean$depth_cat=="shallow",]
ocean_deep<-ocean[ocean$depth_cat=="deep",]

# pull out the data where we have NAs in the temp
settle_ocean_temp<-settle_ocean[is.na(settle_ocean$temp_c),]
summary(settle_ocean_temp)

# pull in first the shallow temp and then the deep temp
settle_ocean_temp$temp_c<-ifelse(is.na(settle_ocean_temp$temp_c),
                                 ocean_shallow$temp_c[match(ocean_shallow$ID,settle_ocean_temp$ID)],NA)
settle_ocean_temp$temp_c<-ifelse(is.na(settle_ocean_temp$temp_c),
                                 ocean_deep$temp_c[match(ocean_deep$ID,settle_ocean_temp$ID)],NA)
summary(settle_ocean_temp)

# remerge 
settle_ocean<-settle_ocean[!is.na(settle_ocean$temp_c),]
settle_ocean<-rbind(settle_ocean,settle_ocean_temp)

# repeat with both oxygens 
# no data for either oxy measure, so inputting into the full dataset 
summary(settle_ocean)

#oxy_mgl
settle_ocean$oxy_mgl<-ifelse(is.na(settle_ocean$oxy_mgl),
                                 ocean_shallow$oxy_mgl[match(ocean_shallow$ID,settle_ocean$ID)],NA)
settle_ocean$oxy_mgl<-ifelse(is.na(settle_ocean$oxy_mgl),
                                 ocean_deep$oxy_mgl[match(ocean_deep$ID,settle_ocean$ID)],NA) 
summary(settle_ocean)

#oxy_mll
settle_ocean$oxy_mll<-ifelse(is.na(settle_ocean$oxy_mll),
                             ocean_shallow$oxy_mll[match(ocean_shallow$ID,settle_ocean$ID)],NA)
settle_ocean$oxy_mll<-ifelse(is.na(settle_ocean$oxy_mll),
                             ocean_deep$oxy_mll[match(ocean_deep$ID,settle_ocean$ID)],NA) 
summary(settle_ocean)

# in short - borrowing from different depths did not gain me much! 


# EXPORT DATA! WOOHOO! 


#write.csv(settle_ocean,"combined_settlement_updatedocean.csv",row.names = F)
